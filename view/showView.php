<?php
$title = 'Password manager';
ob_start();
?>

<main class="show">

    <section class="user-info">
        <p>
            <span class="email"><?= $_SESSION['user']->email() ?></span>
        </p>
        <a class="deconnexion" href="deconnexion">
            <svg height="18" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="power-off" class="svg-inline--fa fa-power-off fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                <path fill="currentColor" d="M400 54.1c63 45 104 118.6 104 201.9 0 136.8-110.8 247.7-247.5 248C120 504.3 8.2 393 8 256.4 7.9 173.1 48.9 99.3 111.8 54.2c11.7-8.3 28-4.8 35 7.7L162.6 90c5.9 10.5 3.1 23.8-6.6 31-41.5 30.8-68 79.6-68 134.9-.1 92.3 74.5 168.1 168 168.1 91.6 0 168.6-74.2 168-169.1-.3-51.8-24.7-101.8-68.1-134-9.7-7.2-12.4-20.5-6.5-30.9l15.8-28.1c7-12.4 23.2-16.1 34.8-7.8zM296 264V24c0-13.3-10.7-24-24-24h-32c-13.3 0-24 10.7-24 24v240c0 13.3 10.7 24 24 24h32c13.3 0 24-10.7 24-24z"></path>
            </svg>
            Déconnexion
        </a>
    </section>

    <div class="input-search-container">
        <input type="text" placeholder="Rechercher" class="search">
    </div>

    <div class="table-container">
        <?php if (isset($message)) { ?>
            <div class="<?= $message->getType() ?>-message">
                <span class="icon"></span>
                <?= $message->getText() ?>
            </div>
        <?php } ?>

        <div class="info-table">
            <p>
                Nombre de lignes : <strong><?= count($champs) ?></strong>
            </p>

            <a href="add" class="btn btn-primary">
                <i class="fas fa-plus"></i>
                Nouvelle ligne
            </a>
        </div>
        <div class="table">
            <div class="row row--header">
                <a href="modify" class="action vide">Éditer</a>
                <div class="case">Site</div>
                <div class="case">Email</div>
                <div class="case">Nom d'utilisateur</div>
                <div class="case">Mot de passe</div>
                <div class="case">Description</div>
            </div>
            <div class="tbody">
                <?php if (!empty($champs)) {
                    foreach ($champs as $key => $champ) { ?>
                        <div class="row" id="<?= $champ->id() ?>">
                            <a href="modify?id=<?= $champ->id() ?>" class="action">Éditer</a>
                            <div class="case site" data-site="<?= $champ->site() ?>"><?= $champ->site() ?></div>
                            <div class="case email"><?= $champ->email() ?></div>
                            <div class="case username"><?= $champ->username() ?></div>
                            <div class="case password"><?= $champ->password('decrypt') ?></div>
                            <div class="case description"><?= $champ->description() ?></div>
                        </div>
                    <?php } ?>
                <?php } ?>
            </div>
        </div>
    </div>

    <div class="delete-champ">
        <div class="overlay">
            <div class="popup">
                <!-- Bande bleu de titre -->
                <div class="title">
                    <div class="close-popup">
                        <span>&times;</span>
                    </div>
                    <h1>Supprimer un champ</h1>
                </div>
                <div class="content">
                    <p>Etes-vous sur de vouloir supprimer ce champ ?</p>
                    <div class="button-container">
                        <button class="annuler">Annuler</button>
                        <button class="supprimer">Supprimer</button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>

<!-- JS -->
<script src="./js/show.js"></script>

<?php
$content = ob_get_clean();
require 'template.php';
?>